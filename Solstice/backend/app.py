...
from src.mc.gbm import simulate, summary, stress_scenarios
from src.mc.cards import shuffle, deal, hand_probability
from src.mc.sports import poisson_match, elo_expected, simulate_tournament
from src.prob.bayes import BetaBernoulli
from src.prob.hypothesis import probabilistic_sharpe_ratio, deflated_sharpe_ratio
from src.prob.calibration import brier_score
import numpy as np
...
def api_stress():
    d = request.get_json() or {}
    return jsonify(stress_scenarios(float(d.get('spot', 100)), float(d.get('mu', 0.10)), float(d.get('sigma', 0.25))))
...
def api_bayes():
    ...
    return jsonify({'mean': bb.posterior_mean(), 'ci': list(bb.credible_interval()), 'prob_gt_50': bb.probability_greater_than(0.5), 'prior': prior, 'posterior': posterior, 'x': xs.tolist()})

@app.route('/api/prob/hypothesis', methods=['POST'])
def api_hypothesis():
    d = request.get_json() or {}
    returns = d.get('returns', [])
    return jsonify({'psr': probabilistic_sharpe_ratio(returns), 'dsr': deflated_sharpe_ratio(returns, int(d.get('n_trials', 1))), 'brier': brier_score([0.5]*len(returns), [1 if r > 0 else 0 for r in returns]) if returns else 0})
